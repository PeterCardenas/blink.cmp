  ...
  local function get_completions_for_node(node)
    ...
    return async.task.new(function(resolve, reject)
      return node.source:get_completions(context, function(items, is_cached)
        ...
        if should_push_upstream or node.source.name == 'emoji' or node.source.name == 'Path' or node.source.name == 'lsp' then self:emit_completions(items_by_provider, on_items_by_provider) end
